...
class _OpenTradeState extends State<OpenTrade> {
  ...
  getdatabaseData() async {
    ...
    debugPrint(
        "the database data is ${data[0].takeProfit},${data[0].date},${data[0].lotSize},${data[0].stoploss},${data[0].hitby},${data[0].open},${data[0].notes}");
    if (data != null) {
      db = data;
      for (int i = 0; i < db.length; i++) {
        List<String> parts =
            db[i].date.split("-"); // Split the date string by '-'
        String formattedDate = "${parts[2]}-${parts[1]}-${parts[0]}";
        debugPrint("date is ${db[i].date}");
        DateTime date = DateTime.parse(
            DateFormat('yyyy-MM-dd').format(DateTime.parse(formattedDate)));
                            ...
                            child: Container(
                              ...
                              decoration: new BoxDecoration(
                                ...
                                border: new Border.all(
                                    width: 1.0,
                                    color:
                                        true ? Colors.blueAccent : Colors.grey),
